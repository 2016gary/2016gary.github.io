...
(function(window, document, $, undefined) {
			...
			.extend(
					F,
					{
						...
						defaults : {
							...
							maxWidth : 9999,
							maxHeight : 9999,
							pixelRatio : 1, // Set to 2 for retina display
							// support
							...
							// Opening fancyBox
							openEffect : 'fade', // 'elastic', 'fade' or
							// 'none'
							openSpeed : 250,
							openEasing : 'swing',
							openOpacity : true,
							...
							// Closing fancyBox
							closeEffect : 'fade', // 'elastic', 'fade' or
							// 'none'
							closeSpeed : 250,
							closeEasing : 'swing',
							closeOpacity : true,
							...
							nextEffect : 'elastic', // 'elastic', 'fade' or
							// 'none'
							nextSpeed : 250,
							nextEasing : 'swing',
							nextMethod : 'changeIn',
							...
							prevEffect : 'elastic', // 'elastic', 'fade' or
							// 'none'
							prevSpeed : 250,
							prevEasing : 'swing',
							...
							beforeShow : $.noop, // Before changing in
							// current item
							afterShow : $.noop, // After opening
							beforeChange : $.noop, // Before changing gallery
							// item
						...
						isOpened : false, // Have been fully opened at least
						// once
						...
						open : function(group, opts) {
							...
							// Recheck if the type of each element is `object`
							// and set content type (image, ajax, etc)
							$
									.each(
											group,
											function(i, element) {
												...
												if (isString(href)) {
													...
													// Split url into two pieces
													// with source url and
													// content selector, e.g,
													// "/mypage.html #my_id"
													// will load "/mypage.html"
													// and display element
													// having id "my_id"
													if (type === 'ajax') {
						...
						cancel : function() {
							...
							// If the first item has been canceled, then clear
							// everything
							if (!F.current) {
						...
						// Start closing animation if is open; remove
						// immediately if opening/closing
						...
						// Center inside viewport and toggle position type to
						// fixed or absolute if needed
						reposition : function(e, onlyAbsolute) {
							var current = F.current, wrap = current ? current.wrap
									: null, pos;
							...
							if (wrap) {
								...
								if (e && e.type === 'scroll') {
									...
									current.pos = $
											.extend({}, current.dim, pos);
								}
							}
						},

						update : function(e) {
							var type = (e && e.type), anyway = !type
						...
						// Shrink content to fit inside viewport or restore if
						// resized
						...
						bindEvents : function() {
							...
							// Changing document height on iOS devices triggers
							// a 'resize' event,
							// that can change document height... repeating
							// infinitely
							W
									.bind(
							...
							if (keys) {
								D
										.bind(
												...
												function(e) {
													var code = e.which
															|| e.keyCode, target = e.target
															|| e.srcElement;

													// Skip esc key if loading,
													// because showLoading will
													// cancel preloading
													if (code === 27 && F.coming) {
														return false;
													}
								...
								F.wrap
										.bind(
												...
												function(e, delta, deltaX,
														deltaY) {
													var target = e.target
															|| null, parent = $(target), canScroll = false;
						...
						_start : function(index) {
							...
							// Convert margin and padding properties to array -
							// top, right, bottom, left
							margin = coming.margin;
							padding = coming.padding;

							if ($.type(margin) === 'number') {
								coming.margin = [ margin, margin, margin,
										margin ];
							}
							...
							/*
							 * Add reference to the group, so it`s possible to
							 * access from callbacks, example: afterLoad :
							 * function() { this.title = 'Image ' + (this.index +
							 * 1) + ' of ' + this.group.length + (this.title ? ' - ' +
							 * this.title : ''); }
							 */
							...
							// Give a chance for callback or helpers to update
							// coming item (type, title, etc)
							F.coming = coming;
							...
							if (!type) {
								...
								// If we can not determine content type then
								// drop silently or display next/prev item if
								// looping through gallery
								if (F.current && F.router
										&& F.router !== 'jumpto') {
							...
							$.each([ "Top", "Right", "Bottom", "Left" ],
									function(i, v) {
										coming.skin.css('padding' + v,
												getValue(coming.padding[i]));
									});

							F.trigger('onReady');

							// Check before try to load; 'inline' and 'html'
							// types need content, others - href
						...
						_loadImage : function() {
							// Reset preload image so it is later possible to
						...
						_loadIframe : function() {
							var coming = F.coming, iframe = $(
									coming.tpl.iframe.replace(/\{rnd\}/g,
											new Date().getTime())).attr(
									'scrolling',
						...
						_afterLoad : function() {
							...
							if (false === F.trigger('afterLoad', coming,
									previous)) {
								coming.wrap.stop(true).trigger('onReset')
										.remove();
							...
							// Give a chance for helpers or callbacks to update
							// elements
							...
							if (!F.isOpened) {
								$('.fancybox-wrap').not(current.wrap)
										.stop(true).trigger('onReset').remove();
						...
						_setDimension : function() {
							var viewport = F.getViewport(), steps = 0, canShrink = false, canExpand = false, wrap = F.wrap, skin = F.skin, inner = F.inner, current = F.current, width = current.width, height = current.height, minWidth = current.minWidth, minHeight = current.minHeight, maxWidth = current.maxWidth, maxHeight = current.maxHeight, scrolling = current.scrolling, scrollOut = current.scrollOutside ? current.scrollbarWidth
									: 0, margin = current.margin, wMargin = getScalar(margin[1]
									+ margin[3]), hMargin = getScalar(margin[0]
							...
							wPadding = getScalar(skin.outerWidth(true)
									- skin.width());
							hPadding = getScalar(skin.outerHeight(true)
									- skin.height());

							// Any space between content and viewport (margin,
							// padding, border, title)
							...
							if (current.type === 'iframe') {
								...
								// Set width or height in case we need to
								// calculate only one dimension
							...
							// These will be used to determine if wrap can fit
							// in the viewport
						...
						_getPosition : function(onlyAbsolute) {
							var current = F.current, viewport = F.getViewport(), margin = current.margin, width = F.wrap
									.width()
									+ margin[1] + margin[3], height = F.wrap
									.height()
									+ margin[0] + margin[2], rez = {
								position : 'absolute',
								top : margin[0],
								left : margin[3]
							};
							...
							if (current.autoCenter && current.fixed
									&& !onlyAbsolute && height <= viewport.h
									&& width <= viewport.w) {
								rez.position = 'fixed';
						...
						_afterZoomIn : function() {
							...
							if (!current.loop
									&& current.index === current.group.length - 1) {
								F.play(false);
	...
	F.helpers.overlay = {
		defaults : {
			...
			showEarly : true, // indicates if should be opened immediately or
			// wait until the content is ready
			css : {}, // custom CSS properties
			locked : !isTouch, // if true, the content will be locked into
			// overlay
			fixed : true
		// if false, the overlay CSS position property will not be set to
		// "fixed"
		},
		...
		// This is where we can manipulate DOM, because later it would cause
		// iframes to reload
	...
	F.helpers.title = {
		...
		beforeShow : function(opts) {
			...
			default: // 'float'
				...
				// Increase bottom margin so this title will also fit into
				// viewport
	...
	$.fn.fancybox = function(options) {
		var index, that = $(this), selector = this.selector || '', run = function(
				e) {
				...
				if ($.scrollbarWidth === undefined) {
					...
					$.scrollbarWidth = function() {
						var parent = $(
								...
								.appendTo('body'), child = parent.children(), width = child
								.innerWidth()
				...
				if ($.support.fixedPosition === undefined) {
					$.support.fixedPosition = (function() {
						var elem = $(
								...
								.appendTo('body'), fixed = (elem[0].offsetTop === 20 || elem[0].offsetTop === 15);
